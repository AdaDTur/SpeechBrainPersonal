#!/usr/bin/env python3
"""Recipe for training an encoder and decoder model for audio
...
class MNISTBrain(sb.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        predictions = self.modules.decoder(embeddings)
    ...
    def prepare_features(self, wavs, stage):
        ...
        size = feats.shape[1]
        feats_pad = F.pad(feats, pad=(0, 0, 0, 100 - size, 0, 0))
        feats_pad = torch.unsqueeze(feats_pad, dim=1)
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        loss = sb.nnet.losses.mse_loss(predictions, data, lens)

        return loss

    def on_stage_start(self, satge, epoch=None):
...
def dataio_prep(hparams):
    ...
    def label_pipeline(digit):
        """Defines the pipeline to process the input."""
        yield digit
